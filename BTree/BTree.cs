...
namespace BTree
{
    public partial class BTreeForm : Form
    {
        ...
        private void Exercise3__Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 15; ++i)
            {
                // Generate a random uppercase string of length 5 
                string randomString = new string(Enumerable.Repeat("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 5)
                    .Select(s => s[random.Next(s.Length)]).ToArray());

                // Insert the string into the binary tree
        ...
        private void Exercise4__Click(object sender, EventArgs e)
        {
            ...
            // Prime the tree using the code from PrimedButton__Click
            node = new BTree("M", null);
            root = node;
        ...
        private void Exercise5__Click(object sender, EventArgs e)
        {
            ...
            for (char c = 'A'; c <= 'Z'; c++)
            {
                node = new BTree(c.ToString(), root);
        ...
        private void Exercise6__Click(object sender, EventArgs e)
        {
            ...
            List<Person> familyMembers = new List<Person>
            {
                ...
                new Person("Mom", 45),
                new Person("Teddy", 8), 
                new Person("Grandpa", 80), 
                new Person ("Grandma", 75), 
                new Person ("Cousin1", 13),
                new Person ("Cousin2", 8),
                new Person ("Cousin3", 3), 
                new Person ("Uncle1", 40), 
                new Person ("Aunt1", 38), 
                new Person ("Aunt2", 49),
                new Person ("Uncle2", 51),
                new Person ("Grandma2", 80),
                new Person ("Cousin4", 20),
                new Person ("Cousin5", 15)

          
            };

            // Insert family members into the tree starting with yourself
        ...
        private void Exercise7__Click(object sender, EventArgs e)
        {
            ...
            foreach (Person member in familyMembers)
            {
                BTree newNode = new BTree(member, null);
                if (root == null)
                {
                    root = newNode;
                }
