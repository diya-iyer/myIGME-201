...
namespace Unit1Exam_Q1
{
    ...
    // Class: Program
    // Author: Diya Iyer
    // Purpose: Hosting the main method that contains a math quiz
    // Restrictions: None
    class Program
        // Method: Main 
        // Author: Diya Iyer
        // Purpose: Modifying the existing math quiz in order to add division with double values
        // Restrictions: None
    {
        static void Main()
        {
            ...
            string sResponse = "";
            Int32 nResponseInt = 0;
            double nResponseDbl = 0.00;
        ...
        start:
            ...
            for (nCntr = 0; nCntr < nQuestions; ++nCntr)
            {
                ...
                // changing the range to 0 inclusive and 4 exclusive to accomodate the extra operation of division 
                nOp = rand.Next(0, 4);
                ...
                {
                    nAnswer = val1 + val2;
                    sQuestions = $"Question #{nCntr + 1} (Please enter an integer) : {val1} + {val2} => ";
                }
                else if (nOp == 1)
                {
                    nAnswer = val1 - val2;
                ...
                {
                    // cast the integer values to doubles to get the proper decimal places and round it to the hundredths place
                    dAnswer = (double)val1 / val2;
                    dAnswer = Math.Round(dAnswer, 2);
                    sQuestions = $"Question #{nCntr + 1} (Please enter a numeric value with two decimal places) : {val1} / {val2} => ";
                }
                ...
                do
                {
                    ...
                    try
                    {
                        // Parse the double value for division and integer for all other operations
                        if (nOp > 2)
                        {
                            nResponseDbl = double.Parse(sResponse);
                            nResponseDbl = Math.Round(nResponseDbl, 2);
                        }
                        else
                        {
                            nResponseInt = int.Parse(sResponse);
                        }
                    ...
                    catch
                    {
                        if (nOp > 2)
                        {
                            Console.WriteLine("Please enter a numeric value with two decimal places.");
                        }
                        else
                        {
                            Console.WriteLine("Please enter an integer.");
                ...
                // If division, compare the double value answer, integer for all other operations
                if (nOp > 2)
                {
                    if (nResponseDbl == dAnswer)
                    {
